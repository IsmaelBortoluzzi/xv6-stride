...
// Jump here with ptable.lock on
struct proc *
find_smallest(void)
{
  struct proc *p, *smallest = ptable.proc;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    // cprintf("pid: %d ; state: %d ; stride: %d\n", p->pid, p->state, p->stride);
    if(p->state == RUNNABLE){
      smallest = p;
      break;
    }
  ...
  for(; p < &ptable.proc[NPROC]; p++){
    ...
    if(p->stride < smallest->stride)
      smallest = p;
    else if(p->stride == smallest->stride)
      if(p->pid > smallest->pid)
        smallest = p;
  }
...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(count_scheduled() % 500 == 0){
      stest_print();
      cprintf("pid: %d ; pticktes: %d ; stride: %d\n", p->pid, p->tickets, p->stride);
      cprintf("\n");
    }
